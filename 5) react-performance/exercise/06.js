...
const DogInputContext = React.createContext()
...
function DogNameProvider({children}) {
  ...
  return (
    <DogInputContext.Provider value={value}>
      {children}
    </DogInputContext.Provider>
  )
}
...
function useDogState() {
  const context = React.useContext(DogInputContext)
  if (!context) {
...
function withStateSlice(Comp, stateSlice) {
  const MemoComp = React.memo(Comp)
  function Wrapper(props, ref) {
    const state = useAppState()
    return <MemoComp ref={ref} state={stateSlice(state, props)} {...props} />
  }
