...
function usePokemonCacheProvider() {
  const context = React.useContext(PokemonCacheContext)
  if (!context) {
    throw new Error(
      'usePokemonCacheProvider must be used within a PokemonCacheProvider',
    )
  }
  return context
}
...
function PokemonInfo({pokemonName}) {
  const [cache, dispatch] = usePokemonCacheProvider()
...
function PreviousPokemon({onSelect}) {
  const [cache] = usePokemonCacheProvider()
  return (
    <div>
      Previous Pokemon
